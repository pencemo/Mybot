...
const issuesMdg =
  "📝 Use letter below for solving ണ്ട problem; \n\n   ● MLKV & Apple cards  👉  ( @ ) \n   ● FML & MVM  👉 ( ï ) \n   ● Scribe  👉  ( > )\n\n📝 Use letter below for solving സ്റ്റ problem\n\n   ● All fonts 👉 ( Ì ) \n\n\n©️ @pencemodesign";
const downLoad =
  "Download [Code pro app](https://www.amazon.com/dp/B0DFPT4JWG/ref=apps_sf_sta) from Amazon Appstore ";
    ...
    reply_markup: {
      inline_keyboard: [
        [
          {
            text: "🌟 Share Me 🌟",
            url: "http://t.me/share/url?url=https://t.me/Unicodepro_bot",
          },
        ],
    ...
    reply_markup: {
      inline_keyboard: [
        [
          {
            text: "Download 🌟",
            url: "https://www.amazon.com/dp/B0DFPT4JWG/ref=apps_sf_sta",
          },
        ],
        [{ text: "Home", callback_data: "start" }],
      ],
...
const search = async (ctx) => {
  ...
  if (!isNaN(username)) {
    ...
    user = await User.findOne({ chatId: username }).exec();
  } else {
    // username is not a number, treat it as username or first name
    user = await User.findOne({
      $or: [{ username: username }, { firstName: username }],
    }).exec();
  }
  if (user) {
    ...
    ctx.reply(
      `🔎 User found:\n\n● User ID: ${user.chatId} ${
        user.username && `\n● Username: @${username}`
      }\n● Name: [${firstName} ${user.lastName ? lastName : ""}](tg://user?id=${
        user.chatId
...
const deleteuser = async (ctx) => {
  ...
  if (identifiers.length === 0) {
    return ctx.reply(
      "Please provide at least one identifier (username, first name, or chat ID) to delete."
    );
  }
    ...
    try {
      ...
      if (!isNaN(identifier)) {
        ...
        user = await User.findOneAndDelete({
          $or: [{ username: identifier }, { firstName: identifier }],
        }).exec();
      }
